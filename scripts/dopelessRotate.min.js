...
0;var autorotatespeed=settings.autorotatespeed;var speedMult=settings.speedmultiplyer;if(speedMult!=1)speedMult=speedMult/10+1;var hsFade=settings.hotspotsFade;if(hotspots)var toti;var hotspotsTitle=settings.hotspotsTitle;if(nophp){var nophpimglist=settings.nophpimglist;var nophpzoomlist=settings.nophpzoomlist}var fullpath=$(this)[0].src;var a=document.createElement("a");a.href=fullpath;var imgpath=a.pathname+a.search;var thisName=$(this).attr("id");if(playstop)if(isautorotate)$("."+thisName+"."+
playstop+"").addClass("busy");var contWidth=$(this).attr("width");var contHeight=$(this).attr("height");$(this).wrap('<div class="ts_holder" id="holder_'+thisName+'"/>');var doc=$(document);var holder=$("#holder_"+thisName+"");holder.html("<img class='ts_img_view' src='' /><img class='ts_imgzoom_view' src='' />"+pointerDiv+zoomDiv+"<div class='loading_bg'><div class='loading'><p>loading</p><div class='loading_bar'><div class='loading_bar_inside'></div></div></div></div><div class='zoomload_bg'><div class='zoomload_gif'></div></div></div>"+
...
var posiy=$(this).attr("posiy");var title=$(this).attr("title");var nomenu=parseInt($(this).attr("nomenu"));var text=$(this).text();$(this).remove();toti[index]=new Object;toti[index]["frame"]=frame;toti[index]["posix"]=posix;toti[index]["posiy"]=posiy;toti[index]["title"]=title;toti[index]["text"]=text;toti[index]["nomenu"]=nomenu;if(index+1==counthotspots){holder.append('<div class="highlights"><a href="#" class="highlights_but">'+hotspotsTitle+"</a></div>");for(var i=0;i<toti.length;i++)if(toti[i].nomenu!=
1){holder.find(".highlights").append('<a class="highlights_item" href="'+i+'">'+toti[i].title+"</a>");$(".highlights_item").css({"display":"none"})}}})}}if(isautorotate)startautorotate()}if(isautorotate){holder.on("mousedown",function(){stopautorotate()});if(is_touch_device)holder.on("touchstart",function(){stopautorotate()})}if(hotspots){var expanded=false;$(document).on("click",".highlights_but",function(e){e.preventDefault()});holder.find(".ts_img_view").on("click",function(){holder.find(".highlights_item").removeClass("active");
...
frameno){currentFrame++;image.attr("src",imagelist[currentFrame]);if(currentFrame==frameno){startFrame=currentFrame;showHighlights(itemid);setPointer()}else setTimeout(step,30)}})()}function startautorotate(rtl){playing=true;if(!rtl)autorotate=setInterval(nextFrame,autorotatespeed);if(rtl)autorotate=setInterval(prevFrame,autorotatespeed);if(playstop)$("."+thisName+"."+playstop+"").addClass("busy")}function stopautorotate(){clearInterval(autorotate);playing=false;isrotateloop=true;if(playstop)$("."+
thisName+"."+playstop+"").removeClass("busy")}function showHighlights(itemid){holder.find(".expanded").removeClass("expanded").find(".hltitle,.hltext").remove();holder.find(".highlights_item").removeClass("active");for(var i=0;i<toti.length;i++)if(toti[i].frame==currentFrame){holder.append('<div class="hotspot" id="hs'+i+'"></div>');holder.find("#hs"+i+"").css({"top":toti[i].posiy+"%","left":toti[i].posix+"%"}).fadeIn(50);if(toti[i].posix>60)holder.find("#hs"+i+"").addClass("posr");if(toti[i].posiy>
75)holder.find("#hs"+i+"").addClass("posb")}if(itemid!==undefined)expandHighlight(itemid);highlightsHidden=false}function hideHighlights(){if(expanded)collapseHighlight(function(){holder.find(".highlights_item").removeClass("active");if(hsFade)holder.find(".hotspot").fadeOut(150,function(){$(this).remove()});else{holder.find(".hotspot").css({"display":"none"});$(this).remove()}highlightsHidden=true});else{holder.find(".highlights_item").removeClass("active");if(hsFade)holder.find(".hotspot").fadeOut(150,
function(){$(this).remove()});else{holder.find(".hotspot").css({"display":"none"});$(this).remove()}highlightsHidden=true}}function expandHighlight(itemid){if(itemid!==undefined){holder.find(".highlights_item").removeClass("active");holder.find('.highlights_item[href="'+itemid+'"]').addClass("active");holder.find("#hs"+itemid+"").addClass("expanded").append('<span class="hltitle">'+toti[itemid].title+"</span>");if(toti[itemid].text)if(holder.find("#hs"+itemid+"").is(".posb"))holder.find("#hs"+itemid+
"").prepend('<span class="hltext">'+toti[itemid].text+"</span>");else holder.find("#hs"+itemid+"").append('<span class="hltext">'+toti[itemid].text+"</span>");if(holder.find("#hs"+itemid+"").is(".posr")){var hsposition=holder.find("#hs"+itemid+"").position();holder.find("#hs"+itemid+"").css({"left":"0px"});var hswidth=holder.find("#hs"+itemid+"").outerWidth();var newhsposition=hsposition.left-hswidth+20;holder.find("#hs"+itemid+"").css({"left":newhsposition+"px"})}if(holder.find("#hs"+itemid+"").is(".posb")){var hsposition=
holder.find("#hs"+itemid+"").position();holder.find("#hs"+itemid+"").css({"top":"0px"});var hsheight=holder.find("#hs"+itemid+"").outerHeight();var newhsposition=hsposition.top-hsheight+20;holder.find("#hs"+itemid+"").css({"top":newhsposition+"px"})}expanded=true}}function collapseHighlight(callback){var itemid=parseInt(holder.find(".expanded").attr("id").replace("hs",""));holder.find(".expanded").css({"top":toti[itemid].posiy+"%","left":toti[itemid].posix+"%"}).removeClass("expanded").find(".hltitle,.hltext").remove();
expanded=false;if(callback)callback()}}function setPointer(){var corner=Math.floor(360/countFrames)*direction;var degrees=corner*currentFrame;var radians=degrees*Math.PI/180;var sine=Math.sin(radians);var cose=Math.cos(radians);var poinx=rotCenter+rotRadius*sine*-1;var poiny=rotCenter+rotRadius*cose;_css(".pointer",{"left":poinx,"top":poiny})}function rotateImg(enterPosition){doc.on("mousemove.dragrotate",function(e){if(changeAxis)var cursorPosition=e.pageY-contOffset.top;else var cursorPosition=
...
function(e){if(!zoomon){if(changeAxis)var enterPosition=e.pageY-contOffset.top;else var enterPosition=e.pageX-contOffset.left;rotateImg(enterPosition)}});if(rotatehover)holder.on("mouseenter.initrotate",function(e){if(!zoomon){if(changeAxis)var enterPosition=e.pageY-contOffset.top;else var enterPosition=e.pageX-contOffset.left;rotateImg(enterPosition)}});holder.find(".zoom").on("click.initzoom",function(e){var offset=holder.offset();var startXpos=e.pageX-offset.left;var startYpos=e.pageY-offset.top;
zoomImg(startXpos,startYpos,offset)});if(is_touch_device){holder.find(image).on("touchstart.initrotatemob",function(mobileEvent){if(hotspots)$(document).find(".highlights_item").css({"display":"none"});if(!zoomon){mobileEvent.preventDefault();var sevent=window.event;if(changeAxis)var enterPosition=sevent.touches[0].pageY-contOffset.top;else var enterPosition=sevent.touches[0].pageX-contOffset.left;rotateImgMobile(enterPosition)}});holder.find(".zoom").on("touchstart.initzoommob",function(mobileEvent){mobileEvent.preventDefault();
var offset=holder.offset();zoomImgMobile(offset)})}function nextFrame(){if(hotspots)hideHighlights();currentFrame++;if(currentFrame>=countFrames)currentFrame=0;image.attr("src",imagelist[currentFrame]);if(!changeAxis)setPointer();startFrame=currentFrame;if(hotspots)showHighlights();if(!isrotateloop){autorotateFrame=autorotateFrame+1;if(autorotateFrame==countFrames)stopautorotate()}}function prevFrame(){if(hotspots)hideHighlights();currentFrame--;if(currentFrame<0)currentFrame=countFrames;image.attr("src",
imagelist[currentFrame]);if(!changeAxis)setPointer();startFrame=currentFrame;if(hotspots)showHighlights()}if(goright){$("."+thisName+"."+goright+"").on("click",function(){stopautorotate();nextFrame()});$("."+thisName+"."+goright+"").on("mousedown touchstart",function(){stopautorotate();if(!playing){startautorotate();$(this).on("mouseleave mouseup touchend touchmove",function(){stopautorotate()})}})}if(goleft){$("."+thisName+"."+goleft+"").on("click",function(){stopautorotate();prevFrame()});$("."+
thisName+"."+goleft+"").on("mousedown touchstart",function(){stopautorotate();if(!playing){startautorotate(true);$(this).on("mouseleave mouseup touchend touchmove",function(){stopautorotate()})}})}if(playstop)$("."+thisName+"."+playstop+"").on("click",function(){if(playing)stopautorotate();else startautorotate()})}})(jQuery);
